...
graphBegin += "\t\tleng6 [label=\"n def\", style=filled, color=\"gray\", fillcolor=\"gray\"];\n";
...
https.get('https://spreadsheets.google.com/feeds/cells/18VQygZXodUcmkli9RQjccoUY8qUfw6Vf5B9PChnmZKI/1/public/values?alt=json', (resp) => {
  let data = '';
  resp.on('data', (chunk) => {
    data += chunk;
  });
  resp.on('end', () => {
    gSheetDoData(JSON.parse(data));
  });
}).on("error", (err) => {
